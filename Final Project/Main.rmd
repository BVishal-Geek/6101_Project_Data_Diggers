...
1. Scatter plot for Diameter vs Velocity -- use data1 to do this
2. Anova test for Diamter vs velocity w.r.t Hazardous
#we have diameter column as Est Dia in KM MAX
```{r}
ggplot(data1, aes(x = Log_Est_Diameter, y = Relative.Velocity.km.per.sec)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Log Estimated Diameter", y = "Relative Velocity (km/sec)", title = "Scatter Plot of Log Estimated Diameter vs Relative Velocity")

fit <- aov(Log_Est_Diameter ~ Relative.Velocity.km.per.sec * Hazardous, data = data1)
summary(fit)
...
# Train/Test split without sampling 
set.seed(123)
trainIndex <- createDataPartition(data1$Hazardous, times = 1, p = 0.7, list = FALSE)  
data1_train <- data1[trainIndex, ]
data1_test <- data1[-trainIndex, ]

# Train Logistic Regression Model
data1_train$Hazardous <- factor(data1_train$Hazardous) 
logistic_model <- glm(Hazardous ~ ., data = data1_train, family = "binomial")

# Make predictions on training set
train_preds <- predict(logistic_model, newdata = data1_train, type="response")
train_preds <- ifelse(train_preds > 0.5, 1, 0)

# Train accuracy
train_acc <- mean(train_preds == data1_train$Hazardous)
print(paste("Train Accuracy =", round(train_acc, 3)))  

# Make predictions on test set
test_preds <- predict(logistic_model, newdata = data1_test, type="response")
test_preds <- ifelse(test_preds > 0.5, 1, 0)  

# Test accuracy
test_acc <- mean(test_preds == data1_test$Hazardous) 
print(paste("Test Accuracy =", round(test_acc, 3)))

# Plot ROC curve on test set
test_roc <- roc(data1_test$Hazardous, preds)  
auc <- auc(test_roc)
print(paste("Test AUC =", round(auc, 3)))
plot.roc(test_roc, print.auc = TRUE)
...
set.seed(123)
trainIndex <- createDataPartition(dataSmote$Hazardous, times = 1, p = 0.7, list = FALSE) 
dataSmote_train <- dataSmote[trainIndex, ]
dataSmote_test <- dataSmote[-trainIndex, ]

# Train Logistic Regression Model 
dataSmote_train$Hazardous <- factor(dataSmote_train$Hazardous)
logistic_model_smote <- glm(Hazardous ~ ., data = dataSmote_train, family = "binomial")

# Make predictions on training set  
train_preds <- predict(logistic_model_smote, newdata = dataSmote_train, type="response")
train_preds <- ifelse(train_preds > 0.5, 1, 0)  

# Train accuracy
train_acc <- mean(train_preds == dataSmote_train$Hazardous)
print(paste("Train Accuracy =", round(train_acc, 3)))   

# Make predictions on test set 
test_preds <- predict(logistic_model_smote, newdata = dataSmote_test, type="response") 
test_preds <- ifelse(test_preds > 0.5, 1, 0)  

# Test accuracy
test_acc <- mean(test_preds == dataSmote_test$Hazardous)  
print(paste("Test Accuracy =", round(test_acc, 3)))

# Plot ROC Curve on test set
test_roc <- roc(dataSmote_test$Hazardous, test_preds)
auc <- auc(test_roc)  
print(paste("Test AUC =", round(auc, 3)))
plot.roc(test_roc, print.auc = TRUE)
...
First lets run a logistic regression model
...
sample_indices <- sample(1:nrow(data1), 0.8 * nrow(data1))
...
```
There is no effect of hyperparameter tuning.
...
# Compute AUC
roc_result <- roc(testData$Hazardous, probabilities[,2]) # Assuming second column is the positive class
auc(roc_result)
...
accuracy
accuracy3 <- sum(predictions == trainData$Hazardous) / nrow(trainData)
accuracy3



##Random Forest with dataSmote

set.seed(123) # For reproducibility
splitIndex1 <- createDataPartition(data1$Hazardous, p = .70, list = FALSE, times = 1)
trainData1 <- dataSmote[splitIndex,]
testData1 <- dataSmote[-splitIndex,]

# Train the model
rf_model <- randomForest(Hazardous ~ ., data = trainData1)

# Make predictions
predictions <- predict(rf_model, testData1)

# Get probabilities
probabilities <- predict(rf_model, testData1, type="prob")

# Compute AUC
roc_result <- roc(testData1$Hazardous, probabilities[,2]) # Assuming second column is the positive class
auc(roc_result)

# Plot ROC curve
plot(roc_result, main="Smote ROC Curve")

# Calculate accuracy
accuracy <- sum(predictions == testData1$Hazardous) / nrow(testData1)
accuracy
accuracy1 <- sum(predictions == trainData1$Hazardous) / nrow(trainData1)
accuracy1
