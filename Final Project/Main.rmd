...
data1_train$Hazardous <- as.factor(data1_train$Hazardous) 
...
preds_test <- predict(logistic_model, newdata = data1_test , type="response")

# Calculate test accuracy
test_acc <- mean((preds_test > 0.5) == (data1_test$Hazardous))
print(paste("Test Accuracy =", round(test_acc, 3)))

# Make predictions on train set  
preds_train <- predict(logistic_model, newdata = data1_train , type="response")

# Calculate train accuracy
train_acc <- mean((preds_train > 0.5) == (data1_train$Hazardous))
print(paste("Train Accuracy =", round(train_acc, 3)))

# Plot ROC curve on test set 
test_roc <- roc(data1_test$Hazardous, preds_test)
auc <- auc(test_roc)
...
# Train Model 
dataSmote_train$Hazardous <- as.factor(dataSmote_train$Hazardous)
...
preds_test <- predict(logistic_model_smote, newdata = dataSmote_test, type = "response")  

# Test accuracy 
test_acc <- mean((preds_test > 0.5) == (dataSmote_test$Hazardous))
print(paste("Test Accuracy =", round(test_acc, 3)))

# Predictions on train set
preds_train <- predict(logistic_model_smote, newdata = dataSmote_train, type = "response")

# Train accuracy
train_acc <- mean((preds_train > 0.5) == (dataSmote_train$Hazardous))
print(paste("Train Accuracy =", round(train_acc, 3)))

# Test ROC Curve
test_roc <- roc(dataSmote_test$Hazardous, preds_test)
auc <- auc(test_roc) 
print(paste("Test AUC =", round(auc, 3)))
plot.roc(test_roc, print.auc = TRUE)
