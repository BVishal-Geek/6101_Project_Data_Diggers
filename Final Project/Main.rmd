...
Filtered_data$Orbit.Uncertainity <- as.factor(Filtered_data$Orbit.Uncertainity)
subset <- as.data.frame(lapply(Filtered_data, as.numeric))
#correlation <- corrplot(cor(subset), addCoef.col = "white", number.cex = 0.8, diag=FALSE, number.digits =  1, bg = "grey", outline = "black", mar = c(1,1,1,1))
width <- 10
height <- 8
margins <- c(1, 1, 1, 1) 
corrplot(
  ...
  width = width,
  height = height,
  tl.cex = 0.7,  # Adjust the label size
...
### Step : 3: Checking for distribution using Boxplots: 

```{r, include=TRUE, echo=TRUE, results='markup'}


boxPlots <- function(column){
  Plot <- ggplot(Filtered_data, aes(x = !!as.symbol(column), fill = !!as.symbol(column))) +
    geom_boxplot() + ggtitle("Box Plots")
  return(Plot)
}
