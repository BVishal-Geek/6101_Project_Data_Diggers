...
histplots <- function(column) {
  Plot <- ggplot(Filtered_data, aes_string(x = column, group = "1")) +
    geom_histogram(color="green",bins=30)  + ggtitle("Hist Plots", column )
  return(Plot)
...
data1 <- reduced_df[, -which(names(reduced_df) %in% columns_to_remove)]
dataSmote <- reduced_df[, -which(names(reduced_df) %in% columns_to_remove)]
```
...
ggplot(data1, aes(x = Hazardous, fill = Hazardous)) +
  geom_bar() +
  labs(title = 'Distribution of Hazardous Asteroids (0: Not Hazardous, 1: Hazardous)',
       x = 'Hazardous',
       y = 'Count') +
...
ggplot(dataSmote, aes(x = Hazardous, fill = Hazardous)) +
  geom_bar() +
  labs(title = 'After Distribution of Hazardous Asteroids (0: Not Hazardous, 1: Hazardous)',
       x = 'Hazardous',
       y = 'Count') +
  scale_fill_manual(values = c('skyblue', 'salmon')) +
  theme_minimal()
...
3. Logistic regression with data 1(with sampling)
